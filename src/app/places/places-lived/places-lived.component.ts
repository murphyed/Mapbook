...
export class PlacesLivedComponent implements AfterViewInit {
  display = false;
  displayActive(i) {
  ...
  boxNumbers() {
    if (this.userService.userPR !== null) {
      const boxArray = [];
      for (let i = 0; i < this.userService.userPR.length * 2; i++) {
        boxArray.push(i);
      }
      return boxArray;
    }
  ...
  ngAfterViewInit() {
    if (this.userService.userPR.length !== 0) {
      const pastAddresses: any = this.userService.userPR;
